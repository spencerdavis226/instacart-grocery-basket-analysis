# Instacart Grocery Basket Analysis

End-to-end data cleaning, feature engineering, customer segmentation, and stakeholder-focused reporting using real-world grocery transaction data.

---

## Business Problem

Instacart aims to move beyond broad marketing efforts and implement targeted campaigns based on customer behavior. Key stakeholder questions include:

- When does demand peak (by day and hour)?
- Do customers behave differently by region and profile?
- How should promotions vary based on purchasing patterns and price sensitivity?

This analysis transforms raw transactional data into behavioral insights that support campaign timing decisions, segmentation strategy, and operational planning.

---

## Start Here (Primary Deliverable)

**Final Report (Excel):** `report/Instacart_Final_Report.xlsx`

This is the primary stakeholder artifact. It contains:

- The cleaned analytic dataset
- Segmentation outputs
- Visual summaries
- Clear business recommendations

If reviewing only one file, begin here.

---

## Executive Summary

`report/Instacart Executive Summary.pdf`

A concise overview of the objective, methodology, key findings, and stakeholder recommendations.

---

## Key Visual Insights

### Customer Profile by Region

![Customer Profile by Region](assets/cust_profile_by_region.png)

Customer types vary meaningfully across regions, supporting region-specific targeting strategies and promotional tailoring.

---

### Order Frequency by Day of Week

![Order Frequency by Day of Week](assets/order_freq_dow.png)

Weekend demand dominates, with Saturday and Sunday representing the highest order volumes. These windows are optimal for broad promotional campaigns.

---

### Order Frequency by Hour of Day

![Order Frequency by Hour of Day](assets/order_freq_hour_of_day.png)

Orders peak between 10:00 and 15:00, indicating strong late-morning to afternoon purchasing behavior. This period represents prime visibility for high-impact advertising.

---

## Project Overview

This project converts raw Instacart order, product, and customer tables into a structured analytic dataset and explores:

- Shopping peaks (day-of-week and hour-of-day)
- Department and product performance
- Customer loyalty and reorder behavior
- Spending patterns and price sensitivity
- Regional and demographic segmentation

The objective is to translate behavioral data into actionable marketing and operational strategy.

---

## Methodology

1. **Data Cleaning and Validation**
   - Removed duplicates and null values
   - Standardized identifier data types
   - Verified merge integrity using merge flags

2. **Feature Engineering**
   - Price bands
   - Loyalty flags
   - Spending behavior segments
   - Order frequency classifications
   - Regional and demographic profiles

3. **Aggregation and Analysis**
   - Groupby transformations
   - Crosstab segmentation
   - Time-based demand analysis

4. **Visualization and Reporting**
   - Histograms, bar charts, line plots, and scatterplots
   - Stakeholder-focused Excel reporting

---

## Repository Structure

```
instacart-grocery-basket-analysis
│
├── assets/                 # Exported visualizations used in README
├── course_notebooks/       # Archived coursework notebooks
├── notebooks/              # Clean analysis notebooks
├── report/                 # Final stakeholder deliverables
├── src/                    # Production-ready Python scripts
├── tools/                  # Utility scripts
├── .gitignore
└── requirements.txt
```

---

## Reproducibility

Raw datasets are excluded due to size constraints.

To reproduce the analysis:

1. Place original Instacart CSV files in:
   ```
   02 Data/Original Data/
   ```

2. Run from the project root:
   ```bash
   python src/01_build_analytic_dataset.py
   python src/02_analysis_and_insights.py
   ```

Prepared datasets will be saved to `02 Data/Prepared Data/`.

---

## Tools and Skills Demonstrated

- Python (pandas, NumPy)
- Data cleaning and integrity validation
- Feature engineering
- Customer segmentation
- Behavioral analytics
- Data visualization (matplotlib, seaborn)
- Stakeholder reporting and business translation

---

## Business Impact

This project demonstrates the ability to:

- Translate raw transactional data into strategic insights
- Identify high-value customer segments
- Recommend optimal campaign timing
- Support targeted marketing decisions using behavioral evidence

---

For questions regarding methodology or analysis approach, feel free to connect.
